...
import { getHomeAppRoot, getProjectAppRoot } from '../utils/app-paths.js';
...
const SKILL_DIRECTORIES: { path: string; source: SkillSource }[] = [
  ...
  { path: join(getHomeAppRoot(homedir()), 'skills'), source: 'user' },
  { path: join(getProjectAppRoot(process.cwd()), 'skills'), source: 'project' },
];
