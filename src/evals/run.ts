/**
 * LangSmith Evaluation Runner for Kryzov
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps Kryzov agent
// ============================================================================
...
async function correctnessEvaluator({
  ...
  const prompt = `You are evaluating the correctness of an AI assistant's answer to a Kryzov product question.
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const csvPath = path.join(__dirname, 'dataset', 'kryzov_agent.csv');
    ...
    const datasetName = sampleSize 
      ? `kryzov-eval-sample-${sampleSize}-${Date.now()}`
      : 'kryzov-eval';
    ...
    yield {
      ...
      datasetName: sampleSize ? `kryzov_agent (sample ${sampleSize}/${totalCount})` : 'kryzov_agent',
    };
    ...
    if (!dataset) {
      dataset = await client.createDataset(datasetName, {
        description: sampleSize 
          ? `Kryzov evaluation (sample of ${sampleSize})`
          : 'Kryzov evaluation dataset',
      });
    ...
    const experimentName = `kryzov-eval-${Date.now().toString(36)}`;
