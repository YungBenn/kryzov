...
import { getHomeAppRoot } from '../utils/app-paths.js';

const DEFAULT_GATEWAY_PATH = join(getHomeAppRoot(homedir()), 'gateway.json');
...
export function getGatewayConfigPath(overridePath?: string): string {
  return (
    overridePath ??
    process.env.KRYZOV_GATEWAY_CONFIG ??
...
export function resolveWhatsAppAccount(
  ...
  const authDir =
    account.authDir ?? join(getHomeAppRoot(homedir()), 'credentials', 'whatsapp', accountId);
