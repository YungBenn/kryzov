...
type AgentFactory = (config: AgentConfig) => Agent | Promise<Agent>;
...
export class AgentRunnerController {
  ...
  constructor(
    ...
    createAgent: AgentFactory = Agent.create,
  ) {
  ...
  updateConfig(config: Partial<Pick<AgentConfig, 'model' | 'modelProvider' | 'maxIterations'>>) {
    this.agentConfig = {
      ...this.agentConfig,
      ...config,
    };
