...
import { isQuitAlias } from './cli/quit-aliases.js';
...
export async function runCli() {
  ...
  const modelSelection = new ModelSelectionController(
    ...
    (provider, model) => {
      agentRunner.updateConfig({ model, modelProvider: provider });
      lastError = null;
      agentRunner.setError(null);
      refreshError();
    },
  );
  ...
  const handleSubmit = async (query: string) => {
    ...
    const busy =
      modelSelection.isInSelectionFlow() || agentRunner.pendingApproval != null || agentRunner.isProcessing;
    const slashResult = await dispatchSlashCommand({
      query,
    ...
    if (slashResult === 'handled' || slashResult === 'busy') {
      refreshError();
      tui.requestRender();
      return;
    }
  ...
  editor.setAutocompleteProvider(createSlashAutocompleteProvider(slashCommands));
